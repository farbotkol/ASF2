...
* to User record (if email comes from AECOM internal user "@aecom.com")
*
...
trigger sshCaseUserToContact on Case (before insert) {
	...
	if(ssh_User_Contact_Settings__c.getInstance('Default') != null){
		defaultAccountName = ssh_User_Contact_Settings__c.getInstance('Default').Internal_User_Account__c;
	}

	if(defaultAccountName != null){

		List<Account> internalUserAccounts = [select Id, Name from Account where Name = :defaultAccountName];		
