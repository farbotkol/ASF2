...
trigger taskRelatedtoAccountmanager on Account (after Insert,after Update) 
{
    ...
    if(Trigger.isInsert)
    {
        ...
        {
           if(act.Account_Manager__c==null)
               continue;
           
           AECOM_team__c ot = new AECOM_team__c(Account__c = act.Id,Employee__c = act.Account_Manager__c);
    ...
    if(Trigger.isUpdate && !ModifyDepartmentController.NotRunOpportunityUpdateTrigger)
    {
        ...
        for(Account actNew : Trigger.new)
        {
            ...
            if(actNew.Account_Manager__c != Trigger.OldMap.get(actNew.Id).Account_Manager__c)
            {
                actDeptNew.put(actNew.id,actNew.Account_Manager__c);
                actDeptOld.put(actNew.id,Trigger.OldMap.get(actNew.Id).Account_Manager__c);
                actDeptNew1.put(actNew.Id,new AECOM_team__c(Account__c = actNew.Id,Employee__c = actNew.Account_Manager__c,Role__c = 'Account Manager'));
            }
        }
        if(actDeptOld.keyset().size()>0)
        {
            for(AECOM_team__c opd:[Select id,Account__c,Employee__c,Status__c, Role__c 
                                               from AECOM_team__c 
                                               Where Account__c in :actDeptOld.Keyset() and 
                                               (
                                                //Employee__c in :OppDeptNew.Values() OR
                                                Employee__c in :actDeptOld.Values()
                                               )
                                               AND Role__c = 'Account Manager'
                                               ])
            {
                    opd.Employee__c = actDeptNew.get(opd.Account__c) ;
                    actDeptNew1.put(opd.Account__c,null);
                    if(opd.Employee__c !=null)
                        lstDept.add(opd);
                    else
